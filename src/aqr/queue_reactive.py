...
class ImbalanceQR(QRModel):
    ...
    def _adjust_probabilities(
        self, eps: float, impact: float, exclude_cancels: bool = True
    ) -> pl.DataFrame:
        ...
        impact = np.clip(impact, -0.25, 0.25).item()
        ...
        probabilities = self.probabilities.with_columns(
            pl.when(is_ask)
            .then(pl.col("probability").mul(1 + eps).mul(1 + impact))
            .when(is_bid)
            .then(pl.col("probability").mul(1 - eps).mul(1 - impact))
    ...
    def adjust_event_distrib(
        ...
        probabilities = self._adjust_probabilities(eps, impact, exclude_cancels)
